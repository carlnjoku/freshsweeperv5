...
// JobCard.js – with "Mark as Uncompleted" button
// import React, { useState, useRef } from 'react';
// import {
//   View,
//   Text,
...
// import MarkCleanerUncompletedButton from '../host/MarkCleanerUncompletedButton'; // ✅ import the button
...
// const JobCard = ({ schedules, onImagePress, onRefresh }) => {
//   const [expanded, setExpanded] = useState(false);
//   const rotateAnim = useRef(new Animated.Value(0)).current;
//   const scaleAnim = useRef(new Animated.Value(1)).current;
//   const navigation = useNavigation();
...
//   const getStatusText = () => {
//     if (schedules?.status === "completed" && schedules?.verificationStatus === "Pending Approval") {
//       return tSafe('status_pending_approval', 'Pending Approval');
//     }
...
//   // ✅ Determine if a cleaner can be marked as uncompleted
//   const isCleanerTerminal = (cleaner) => {
//     const status = cleaner?.status?.toLowerCase();
//     return ['pending_review', 'uncompleted', 'completed'].includes(status);
//   };

//   const renderCleanerCard = (cleaner, index) => {
//     const progress = getCleanerProgress(cleaner);
//     const fee = getCleanerFee(cleaner);
//     const terminal = isCleanerTerminal(cleaner);
...
//           >
//             <Ionicons name={getStatusIcon()} size={12} color="#fff" />
//             <Text style={styles.cleanerStatusText}>{progress.percentage}%</Text>
//           </View>
//         </View>
...
//           </View>
//           <Text style={styles.progressText}>
//             {tSafe('tasks_progress', '{completed}/{total} tasks', {
...
//               {cleaner.checklist.rooms.map((room, roomIndex) => (
//                 <View key={roomIndex} style={styles.roomChip}>
//                   <Text style={styles.roomText}>{room.replace('_', ' ')}</Text>
//                 </View>
//               ))}
//             </ScrollView>
//           </View>
...
//         {/* ✅ Mark as Uncompleted Button - shown only if not terminal */}
//         {!terminal && (
//           <View style={styles.markButtonContainer}>
//             <MarkCleanerUncompletedButton
//               scheduleId={schedules?._id}
//               cleanerId={cleaner.cleanerId}
//               cleanerName={`${cleaner.firstname} ${cleaner.lastname}`}
//               onSuccess={onRefresh} // refresh parent list after marking
//             />
//           </View>
//         )}
//       </View>
...
//               <AntDesign name="home" size={20} color={COLORS.primary} />
//               <Text style={styles.title} numberOfLines={1}>
//                 {schedules?.schedule?.apartment_name || tSafe('unknown_property', 'Unknown Property')}
//               </Text>
//             </View>
...
//             <Ionicons name="calendar-outline" size={14} color={COLORS.gray} />
//             <Text style={styles.date}>
//               {tSafe('completed_prefix', 'Completed')}{' '}
//               {moment(schedules?.completed_on?.$date || schedules?.completed_on).format(
...
//               </View>
//               <Text style={styles.cleanersPreviewText}>
//                 {tSafe('cleaners_assigned', '{count} cleaner{s} assigned', {
//                   count: participatingCleaners.length,
//                   s: participatingCleaners.length > 1 ? 's' : '',
//                 })}
//               </Text>
//             </View>
//           )}
...
//             {/* Cleaners Section */}
//             <View style={styles.cleanersSection}>
//               <Text style={styles.sectionTitle}>{tSafe('assigned_cleaners', 'Assigned Cleaners')}</Text>
//               <View style={styles.cleanersList}>
//                 {participatingCleaners.map(renderCleanerCard)}
//               </View>
//             </View>
...
//                   <Ionicons name="time-outline" size={16} color={COLORS.gray} />
//                   <Text style={styles.statText}>
//                     {schedules?.schedule?.total_cleaning_time || 0}{' '}
//                     {tSafe('minutes_abbr', 'min')}
//                   </Text>
//                 </View>
//                 <View style={styles.statItem}>
//                   <Ionicons name="cash-outline" size={16} color={COLORS.gray} />
...
//                   <Ionicons name="location-outline" size={16} color={COLORS.gray} />
//                   <Text style={styles.statText} numberOfLines={1}>
//                     {schedules?.schedule?.address?.split(',')[0] ||
//                       tSafe('unknown_address', 'Unknown address')}
//                   </Text>
//                 </View>
//               </View>
//             </View>
...
//             {/* Photo preview section */}
//             <View style={styles.photoSection}>
//               <Text style={styles.sectionTitle}>{tSafe('cleaning_photos', 'Cleaning Photos')}</Text>
//               <PhotoPreview
//                 checklist={schedules.checklist}
//                 onImagePress={onImagePress}
//               />
...
// import ROUTES from '../../constants/routes';
// import { tSafe } from '../../utils/tSafe';
// import MarkCleanerUncompletedButton from '../host/MarkCleanerUncompletedButton';
...
//   // Navigate to ScheduleDetails
//   const handleViewDetails = () => {
//     if (schedules?._id) {
//       navigation.navigate(ROUTES.cleaner_schedule_details, { scheduleId: schedules._id });
//     }
//   };
...
const JobCard = ({ schedules, onImagePress, onRefresh }) => {
  ...
  const getStatusInfo = (status) => {
    const statusMap = {
      completed: {
        label: tSafe('status_completed', 'Completed'),
        color: COLORS.success,
        icon: 'checkmark-circle-outline',
      },
      pending_review: {
        label: tSafe('status_pending_review', 'Pending Review'),
        color: COLORS.warning,
        icon: 'time-outline',
      },
      approved: {
        label: tSafe('status_approved', 'Approved'),
        color: '#28a745',
        icon: 'checkmark-circle-outline',
      },
      uncompleted: {
        label: tSafe('status_uncompleted', 'Uncompleted'),
        color: COLORS.error,
        icon: 'close-circle-outline',
      },
      cancelled: {
        label: tSafe('status_cancelled', 'Cancelled'),
        color: '#6c757d',
        icon: 'close-circle-outline',
      },
      payment_released: {
        label: tSafe('status_payment_released', 'Payment Released'),
        color: '#17a2b8',
        icon: 'cash-outline',
      },
      in_progress: {
        label: tSafe('status_in_progress', 'In Progress'),
        color: '#007bff',
        icon: 'progress-clock',
      },
      upcoming: {
        label: tSafe('status_upcoming', 'Upcoming'),
        color: '#5ac8fa',
        icon: 'calendar-clock',
      },
      pending_payment: {
        label: tSafe('status_pending_payment', 'Pending Payment'),
        color: '#ffc107',
        icon: 'cash-outline',
      },
    };
    ...
    const defaultStatus = {
      label: status || tSafe('status_unknown', 'Unknown'),
      color: COLORS.gray,
  ...
  const statusInfo = getStatusInfo(schedules?.status);
          ...
          <View style={styles.headerContent}>
            ...
            <View style={styles.headerRight}>
              ...
              <View style={[styles.statusBadge, { backgroundColor: statusInfo.color }]}>
                <Ionicons name={statusInfo.icon} size={12} color="#fff" style={styles.statusIcon} />
