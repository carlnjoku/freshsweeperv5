...
const AccountVerificationGate = () => {
  ...
  useEffect(() => {
    ...
    if (deepLinkCleanerId && deepLinkCleanerId === currentUserId) {
      ...
      // Show success message
      Alert.alert(
        tSafe('payment_setup_complete_title', 'Payment Setup Complete!'),
        tSafe('payment_setup_complete_message', 'Your Stripe account has been successfully connected. You can now receive payments.'),
        [{ 
          text: tSafe('continue', 'Continue'),
          onPress: () => {
            // Refresh to update verification status
            refreshVerificationStatus(true);
          }
  ...
  const handleIDVerificationComplete = async (cleanerId) => {
    try {
      ...
      // Show immediate feedback
      Alert.alert(
        tSafe('id_verification_complete_title', 'ID Verification Complete!'),
        tSafe('id_verification_complete_message', 'Your identity has been successfully verified.'),
        [{ 
          text: tSafe('continue', 'Continue'),
          onPress: async () => {
            // Refresh user data to update verification status
      ...
      console.error('Error handling ID verification complete:', error);
      Alert.alert(tSafe('error', 'Error'), tSafe('failed_update_verification_status', 'Failed to update verification status.'));
    }
  };
  ...
  const handleStripeOnboardingComplete = async () => {
    ...
    try {
      ...
      // Show immediate feedback
      setOnboardingStatus(tSafe('verifying', 'Verifying...'));
      ...
      if (verificationResult.charges_enabled) {
        ...
        // Force immediate UI update
        setOnboardingStatus(tSafe('onboarded', 'Onboarded'));
        ...
        setOnboardingStatus(tSafe('not_onboarded', 'Not Onboarded'));
        Alert.alert(
          tSafe('verification_in_progress_title', 'Verification in Progress'),
          tSafe('verification_in_progress_message', 'Your Stripe account is being verified. This may take a few minutes.'),
          [{ text: tSafe('ok', 'OK') }]
        );
      }
    } catch (error) {
      ...
      setOnboardingStatus(tSafe('not_onboarded', 'Not Onboarded'));
      Alert.alert(
        tSafe('verification_error_title', 'Verification Error'),
        tSafe('verification_error_message', 'There was an issue verifying your Stripe account. Please try again.'),
        [{ text: tSafe('ok', 'OK') }]
      );
    } finally {
      setProcessingDeepLink(false);
  ...
  const refreshVerificationStatus = async (force = false) => {
    try {
      ...
      if (isFullyVerified(updatedUser)) {
        ...
        const newOnboardingStatus = updatedUser.onboarding_completed ? tSafe('onboarded', 'Onboarded') : tSafe('not_onboarded', 'Not Onboarded');
        setOnboardingStatus(newOnboardingStatus);
        setVerifyList(generateVerificationItems(updatedUser));
  ...
  const onboardCleaner = async () => {
    try {
      ...
      if (onboardingStatus === 'Not Onboarded') {
        ...
        const linkRes = await userService.createLinkUrl(response.data.account_id, user._id);
        setOnboardingUrl(linkRes.data.update_link);
        setOnboardingStatus(tSafe('onboarding_in_progress', 'Onboarding in Progress'));
      ...
      console.error('Onboarding error:', error.message);
      Alert.alert(tSafe('error', 'Error'), error.response?.data || tSafe('failed_start_onboarding', 'Failed to start onboarding.'));
    } finally {
      setLoading(false);
    }
  };
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (status.identity_verified === 'verified') {
          // Success!
          Alert.alert(
            tSafe('verification_complete_title', 'Verification Complete!'),
            tSafe('verification_complete_message', 'Your identity has been successfully verified.'),
            [{ 
              text: tSafe('continue', 'Continue'),
              onPress: () => refreshVerificationStatus(true)
            }]
          ...
          // Failed or canceled
          Alert.alert(
            tSafe('verification_failed_title', 'Verification Failed'),
            tSafe('verification_failed_message', 'Please try the verification process again.'),
            [{ text: tSafe('ok', 'OK') }]
          );
          return; // Stop polling
        }
  ...
  const handlePress = async (type) => {
    if (type === 'avatar') addImage();
      ...
      try {
        ...
        console.error('ID verification error:', err.message);
        Alert.alert(tSafe('error', 'Error'), tSafe('failed_start_id_verification', 'Failed to start ID verification. Please try again.'));
      }
    }
  };
  ...
  const addImage = async () => {
    ...
    if (!permission.granted) {
      Alert.alert(
        ...
        tSafe('photo_permission_message', 'We need access to your photos to upload a profile picture.')
      );
      return;
    }
  ...
  const generateVerificationItems = (user) => {
    ...
    // Define translations for dynamic statuses
    const statusTranslations = {
      verified: tSafe('identity_verified', 'Identity Verified'),
      pending: tSafe('verification_in_progress', 'Verification in Progress'),
      requires_input: tSafe('more_info_needed', 'More Info Needed'),
      failed: tSafe('verification_failed', 'Verification Failed'),
      default: tSafe('verify_identity', 'Verify Identity')
    };

    const labelForVerification = hasVerifiedID === 'verified' ? statusTranslations.verified :
                                 hasVerifiedID === 'pending' ? statusTranslations.pending :
                                 hasVerifiedID === 'requires_input' ? statusTranslations.requires_input :
                                 hasVerifiedID === 'failed' ? statusTranslations.failed :
                                 statusTranslations.default;

    const descriptionForVerification = hasVerifiedID === 'verified' 
        ? tSafe('identity_verified_description', 'Your identity has been successfully verified.')
        : hasVerifiedID === 'pending'
        ? tSafe('identity_pending_description', 'Your ID verification is being processed.')
        : hasVerifiedID === 'requires_input'
        ? tSafe('identity_requires_input_description', 'Additional information is needed to verify your identity.')
        : hasVerifiedID === 'failed'
        ? tSafe('identity_failed_description', 'ID verification failed. Please try again.')
        : tSafe('identity_required_description', 'Confirm your ID to get started.');

    return [
      {
        id: 1,
        icon: 'account',
        label: tSafe('upload_profile_photo', 'Upload Profile Photo'),
        description: tSafe('upload_profile_photo_description', 'Helps build trust with hosts.'),
        type: 'avatar',
        isCompleted: hasAvatar,
        isEnabled: true,
      },
      {
        id: 2,
        icon: 'credit-card',
        label: hasOnboarded ? tSafe('payment_setup_complete', 'Payment Setup Complete') : tSafe('set_up_payment', 'Set Up Payment'),
        description: hasOnboarded 
          ? tSafe('payment_setup_complete_description', 'Your Stripe account is connected and ready for payouts.')
          : tSafe('set_up_payment_description', 'Receive your payouts securely through Stripe.'),
        type: 'onboarding',
        isCompleted: hasOnboarded,
        isEnabled: hasAvatar,
        status: hasOnboarded ? 'complete' : 'pending'
      },
      {
        id: 3,
        icon: 'shield-check',
        label: labelForVerification,
        description: descriptionForVerification,
        type: 'verify_identity',
        isCompleted: hasVerifiedID === 'verified',
        isEnabled: hasAvatar && hasOnboarded,
      },
  ...
  if (processingDeepLink) {
    return (
      <View style={styles.center}>
        <ActivityIndicator size="large" color={COLORS.primary} />
        <Text style={{ marginTop: 10, textAlign: 'center' }}>
          {tSafe('verifying_stripe_account', 'Verifying your Stripe account...')}{'\n'}
          <Text style={{ fontSize: 12, color: COLORS.gray }}>
            {tSafe('this_may_take_few_seconds', 'This may take a few seconds')}
          </Text>
        </Text>
      </View>
    );
  ...
  if (loading && !processingDeepLink) {
    return (
      <View style={styles.center}>
        <ActivityIndicator size="large" color={COLORS.primary} />
        <Text style={{ marginTop: 10 }}>{tSafe('checking_account_status', 'Checking your account status...')}</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topBanner}>
        <Text style={styles.welcomeTitle}>{tSafe('almost_done_title', "You're Almost Done")}</Text>
        <Text style={styles.welcomeSubtitle}>
          {tSafe('almost_done_subtitle', 'Just complete the final steps below so you can start accepting jobs with confidence. We\'ve made it quick and easy.')}
        </Text>
        ...
        {/* {(deepLinkStripeAccountId || deepLinkCleanerId) && (
          <View style={styles.deepLinkBanner}>
            ...
            <Text style={styles.deepLinkText}>
              {deepLinkStripeAccountId 
                ? tSafe('processing_stripe_onboarding', 'Processing your Stripe onboarding...') 
                : tSafe('processing_id_verification', 'Processing your ID verification...')}
            </Text>
          </View>
        )} */}
      </View>
      ...
      <View style={{ alignItems: 'center', marginVertical: 10 }}>
        ...
        >
          <MaterialIcons name="refresh" size={16} color={COLORS.primary} />
          <Text style={styles.refreshText}>{tSafe('refresh_status', 'Refresh Status')}</Text>
        </TouchableOpacity>
      </View>
