...
const History = ({ schedules, isLoading = false }) => {
  const [selectedFilter, setSelectedFilter] = useState(tSafe('filter_all', 'All'));
  const [isImageViewerVisible, setIsImageViewerVisible] = useState(false);
  const [images, setImages] = useState([]);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  ...
  const renderEmptyState = (type = 'general') => {
    const messages = {
      general: {
        icon: 'history',
        title: tSafe('empty_no_history_title', 'No Cleaning History Yet'),
        message: tSafe('empty_no_history_message', 'Your completed cleaning jobs will appear here once you finish and the host confirms them.')
      },
      filtered: {
        icon: 'filter-variant',
        title: tSafe('empty_no_history_match_title', 'No History Matches Your Filter'),
        message: tSafe('empty_no_history_match_message', 'Try selecting a different time period or clear your filters to see all your completed cleanings.')
      },
      search: {
        icon: 'magnify',
        title: tSafe('empty_no_jobs_found_title', 'No Jobs Found'),
        message: tSafe('empty_no_jobs_found_message', 'No completed jobs match your search. Try using different keywords or dates.')
      },
      error: {
        icon: 'alert-circle-outline',
        title: tSafe('empty_unable_to_load_history_title', 'Unable to Load History'),
        message: tSafe('empty_unable_to_load_history_message', 'There was a problem loading your cleaning history. Please pull down to refresh.')
      }
    };
    ...
    return (
      ...
      >
        ...
        {/* Optional call-to-action button – commented out but could be added later */}
        {/* <TouchableOpacity style={styles.actionButton}>
          <MaterialCommunityIcons name="refresh" size={16} color="#fff" />
          <Text style={styles.actionButtonText}>Refresh History</Text>
        </TouchableOpacity> */}
  ...
  if (isLoading) {
    return (
      <View style={styles.loadingContainer}>
        <ActivityIndicator size="large" color={COLORS.primary} />
        <Text style={styles.loadingText}>{tSafe('loading_history', 'Loading completed jobs...')}</Text>
      </View>
    );
  }
