...
export default function ChatConversation({ navigation, route }) {
  ...
  /**
   * Check if message contains contact information (phone, email, social media)
   */
  const containsContactInfo = (text) => {
    ...
    const patterns = [
      ...
      // Phone numbers: US style with dashes/dots/spaces
      /\b\d{3}[-.]?\s?\d{3}[-.]?\s?\d{4}\b/,
      // Phone: with parentheses
      /\(\d{3}\)\s?\d{3}[-.]?\s?\d{4}/,
      // Phone: international with + and digits
      /\+\d{1,3}\s?\d{1,14}\b/,
      ...
      // Social media URLs
      /(instagram\.com|twitter\.com|facebook\.com|linkedin\.com|t\.co|t\.me|whatsapp\.com|wa\.me|signal\.me|telegram\.me)/i,

      // Platform names followed by handle (e.g., "ig: username", "instagram @user")
      /\b(ig|instagram|fb|facebook|twitter|x|linkedin|whatsapp|signal|telegram|t\.me|wa\.me)\s*[:\-]?\s*@?[a-zA-Z0-9_]+/i,

      // Generic sequence of digits that might be a phone (10-15 digits)
      /\b\d{10,15}\b/
    ];
...
//       Alert.alert(
//         'Contact Information Not Allowed',
//         'Please do not share phone numbers, email addresses, or social media handles in the chat. Keep all communication within the app for your safety and privacy.'
//       );
//       return;
//     }
