...
const CancellationDetails = () => {
  ...
  useEffect(() => {
    const fetchCancellationDetails = async () => {
      try {
        ...
        Alert.alert(tSafe('error_title', 'Error'), tSafe('failed_load_cancellation_details', 'Failed to load cancellation details'));
        setLoading(false);
      }
    };
  ...
  const handleReorder = () => {
    Alert.alert(
      tSafe('reorder_service_title', 'Reorder Service'),
      tSafe('reorder_service_message', 'Would you like to book a new cleaning service?'),
      [
        { text: tSafe('cancel', 'Cancel'), style: 'cancel' },
        {
          text: tSafe('book_now', 'Book Now'),
          onPress: () => navigation.navigate('Booking', {
            previousBooking: cancellationData
          })
        },
  ...
  const handleContactSupport = () => {
    // In a real app, this would open email or chat
    Alert.alert(tSafe('contact_support', 'Contact Support'), tSafe('support_assistance', 'Customer support will be happy to assist you.'));
  };
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        <MaterialCommunityIcons name="refresh" size={48} color="#007AFF" />
        <Text style={styles.loadingText}>{tSafe('loading_cancellation_details', 'Loading cancellation details...')}</Text>
      </View>
    );
  }
  ...
  if (!cancellationData) {
    return (
      <View style={styles.errorContainer}>
        <MaterialCommunityIcons name="alert-circle" size={64} color="#FF3B30" />
        <Text style={styles.errorText}>{tSafe('cancellation_details_not_found', 'Cancellation details not found')}</Text>
        <TouchableOpacity
          style={styles.retryButton}
          onPress={() => navigation.goBack()}
        >
          <Text style={styles.retryButtonText}>{tSafe('go_back', 'Go Back')}</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{tSafe('cancellation_details_title', 'Cancellation Details')}</Text>
        <View style={styles.placeholder} />
      </View>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.statusCard}>
            <View style={styles.statusHeader}>
              ...
              />
              <View style={styles.statusTextContainer}>
                <Text style={styles.statusTitle}>{tSafe('booking_cancelled', 'Booking Cancelled')}</Text>
                <Text style={styles.statusSubtitle}>
                  {cancellationData.refundStatus === 'processed'
                    ? tSafe('refund_processed', 'Refund processed successfully')
                    : cancellationData.refundStatus === 'pending'
                    ? tSafe('refund_pending', 'Refund pending processing')
                    : tSafe('no_refund_applicable', 'No refund applicable')
                  }
                </Text>
              </View>
            </View>
          ...
          {/* Cancellation Details */}
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>{tSafe('cancellation_information', 'Cancellation Information')}</Text>

            <View style={styles.detailRow}>
              <View style={styles.detailLabelContainer}>
                <MaterialCommunityIcons name="calendar" size={16} color="#666" />
                <Text style={styles.detailLabel}>{tSafe('original_booking', 'Original Booking')}</Text>
              </View>
              <Text style={styles.detailValue}>
                {new Date(cancellationData.bookingDate).toLocaleDateString('en-US', {
                  weekday: 'long',
            ...
            <View style={styles.detailRow}>
              <View style={styles.detailLabelContainer}>
                <MaterialCommunityIcons name="clock-outline" size={16} color="#666" />
                <Text style={styles.detailLabel}>{tSafe('cancelled_on', 'Cancelled On')}</Text>
              </View>
              <Text style={styles.detailValue}>
                {new Date(cancellationData.cancelledAt).toLocaleDateString('en-US', {
                  month: 'short',
            ...
            <View style={styles.detailRow}>
              <View style={styles.detailLabelContainer}>
                <MaterialCommunityIcons name="account" size={16} color="#666" />
                <Text style={styles.detailLabel}>{tSafe('cancelled_by', 'Cancelled By')}</Text>
              </View>
              <Text style={styles.detailValue}>
                {cancellationData.cancelledByName}
              </Text>
            ...
            <View style={styles.detailRow}>
              <View style={styles.detailLabelContainer}>
                <MaterialCommunityIcons name="comment-outline" size={16} color="#666" />
                <Text style={styles.detailLabel}>{tSafe('reason', 'Reason')}</Text>
              </View>
              <Text style={styles.detailValue}>
                {cancellationData.reason}
              </Text>
          ...
          {/* Service Details */}
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>{tSafe('service_details', 'Service Details')}</Text>
          ...
          {/* Payment Details */}
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>{tSafe('payment_summary', 'Payment Summary')}</Text>

            <View style={styles.paymentCard}>
              <View style={styles.paymentRow}>
                <Text style={styles.paymentLabel}>{tSafe('original_price', 'Original Price')}</Text>
                <Text style={styles.paymentValue}>
                  ${cancellationData.originalPrice.toFixed(2)}
                </Text>
              </View>
              ...
              {cancellationData.cancellationFee > 0 && (
                <View style={styles.paymentRow}>
                  <Text style={styles.paymentLabel}>{tSafe('cancellation_fee', 'Cancellation Fee')}</Text>
                  <Text style={styles.paymentValue}>
                    -${cancellationData.cancellationFee.toFixed(2)}
                  </Text>
                </View>
              ...
              {cancellationData.refundAmount > 0 && (
                <View style={[styles.paymentRow, styles.refundRow]}>
                  <Text style={styles.refundLabel}>{tSafe('refund_amount', 'Refund Amount')}</Text>
                  <Text style={styles.refundValue}>
                    ${cancellationData.refundAmount.toFixed(2)}
                  </Text>
                </View>
          ...
          {cancellationData.detailedReason && (
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>{tSafe('additional_notes', 'Additional Notes')}</Text>
              <View style={styles.notesCard}>
                <Text style={styles.notesText}>
                  {cancellationData.detailedReason}
                </Text>
          ...
          <View style={styles.actionButtons}>
            ...
            >
              <MaterialCommunityIcons name="calendar-plus" size={20} color="#FFFFFF" />
              <Text style={styles.primaryButtonText}>{tSafe('book_new_service', 'Book New Service')}</Text>
            </TouchableOpacity>
            ...
            >
              <MaterialCommunityIcons name="help-circle" size={20} color="#007AFF" />
              <Text style={styles.secondaryButtonText}>{tSafe('contact_support', 'Contact Support')}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </ScrollView>
