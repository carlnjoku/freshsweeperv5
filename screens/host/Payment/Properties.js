...
export default function Properties({ navigation }) {
  ...
  const fetchProperties = async () => {
    try {
      ...
      console.log(error);
      // Could add an alert here with tSafe if desired
    } finally {
      setLoading(false);
    }
  };
  ...
  const emptyApartment = () => (
    <View style={styles.empty_apartment}>
      <Text>{tSafe('no_apartment_found', 'No apartment found')}</Text>
      <TouchableOpacity
        style={styles.action_button}
        onPress={() => navigation.navigate(ROUTES.host_add_apt)}
      >
        <Text style={styles.action_button_color}>
          {tSafe('create_new_property', 'Create new property')}
        </Text>
      </TouchableOpacity>
    </View>
  );
    ...
    return (
      <View style={styles.noData}>
        <Text style={styles.noDataText}>
          {tSafe('no_properties_listed', 'No properties listed.')}
        </Text>
        <FloatingButton
          onPress={handleOpenCreateBooking}
          color="green"
