  ...
  if (loading && !userData) {
    return (
      <View style={styles.loadingContainer}>
        <Fallback 
          type="loading"
          title={tSafe('loading_profile_title', 'Loading Profile')}
          message={tSafe('loading_profile_message', 'Please wait while we load your profile...')}
        />
        {retryAttempts > 0 && (
    ...
    return (
      <View style={styles.container}>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <Fallback 
            type={error.includes('network') ? 'network-error' : 'error'}
            title={tSafe('failed_load_profile_title', 'Failed to Load Profile')}
            message={error}
            onRetry={refetch}
            showRetry={true}
          />
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {editMode ? (
          ...
          <Button 
            ...
            style={styles.button}
          >
            {tSafe('edit_profile', 'Edit Profileq')}
          </Button>
        )}
