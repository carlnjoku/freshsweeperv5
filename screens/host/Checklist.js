...
export default function Checklist() {
  ...
  useFocusEffect(
    useCallback(() => {
      const { status, message, refresh } = route.params || {};

      // 1️⃣ If refresh is true, reload the list and clear the param
      if (refresh) {
        fetchChecklists();
      ...
      // 2️⃣ If status/message from delete (legacy), show alert
      if (status === 'success' && message && !alertShownRef.current) {
        alertShownRef.current = true;
          ...
          <Card.Content style={styles.cardContent}>
            <View style={styles.cardHeader}>
              ...
              <TouchableOpacity
                ...
                onPress={() => handleDeleteChecklist(checklist.checklistName, checklist._id)}
                activeOpacity={0.7}
              >
...
const styles = StyleSheet.create({
  ...
  menuBackdrop: {
    position: 'absolute',
    top: -500,            // 🟢 larger negative values
    left: -500,
    right: -500,
    bottom: -500,
  ...
  menuCard: {
    ...
    shadowOpacity: 0.15,
    shadowRadius: 12,
    elevation: 15,        // 🟢 higher than container
