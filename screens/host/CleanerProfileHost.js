...
import React, { useContext, useRef, useEffect, useState } from 'react';
// import Text from '../../components/Text';
import { SafeAreaView, StyleSheet, StatusBar, Text, RefreshControl, Linking, Dimensions, FlatList, ScrollView, Image, View, useWindowDimensions, TouchableOpacity, ActivityIndicator, Alert } from 'react-native';
import Button from '../../components/shared/Button';
import {
  get,
  ref,
...
const CleanerProfileHost = ({ navigation, route }) => {
  ...
  const onAddFriend = async uid => {
    ...
    try {
      ...
      if (user) {
        ...
        const automatedMessage = {
          _id: Math.random().toString(36).substring(7), // Generate unique message ID
          text: tSafe('automated_cleaning_request', 'cleaning request!'), // Automated message content
          details: {
            selected_schedule: selected_schedule.schedule,
            selected_scheduleId: selected_scheduleId,
            hostId: fbaseUser.userId,
  ...
  const handleAddFriend = async () => {
    try {
      await onAddFriend(uid, db, fbaseUser, selectedSchedule, selectedScheduleId);
      Alert.alert(tSafe('success_title', 'Success'), tSafe('friend_added_success', 'Friend added successfully'));
    } catch (error) {
      console.error('Error adding friend:', error);
      Alert.alert(tSafe('error_title', 'Error'), tSafe('failed_add_friend', 'Failed to add friend.'));
    }
  };
  ...
  const handleBooking = async () => {
    ...
    await userService.sendCleaningRequest(data)
      .then(response => {
        ...
        const notificationMsg = tSafe('cleaning_request_notification', '{name} sent you a cleaning request', { name: `${currentUser.firstname} ${currentUser.lastname}` });
        // Example usage:
        // sendCleaningRequestPushNotification(
        sendPushNotifications(
          cleaner_tokens, // Replace with a valid Expo Push Token
      ...
      <View style={styles.container}>
        ...
        >
          ...
          <View style={styles.rating_review}>
            ...
            <View>
              {reviews.length > 0 ? <Text onPress={openReviews} style={{ color: COLORS.primary }}>{tSafe('see_all_reviews', 'See all {count} reviews', { count: reviews.length })}</Text>
                :
                ""}
            </View>
          </View>
        ...
        >
          <View style={styles.modalContent}>
            ...
            <View style={styles.modal_header}>
              <View>
                <Text style={styles.header}>{tSafe('customer_reviews', 'Customer Reviews')}</Text>
                <View style={styles.rating}>
                  ...
                  <StarRating
                    ...
                    starStyle={{ marginHorizontal: -1 }} // Customize star spacing
                  />
                  <Text style={{ marginLeft: 5, color: COLORS.gray }}>{calculateOverallRating(reviews, item.cleanerId)}  ({reviews.length} {tSafe('reviews', 'reviews')})</Text>
                </View>
              </View>
              <TouchableOpacity onPress={() => setVisible(false)} style={styles.closeIcon}>
                <MaterialCommunityIcons name="close" size={30} color="gray" />
