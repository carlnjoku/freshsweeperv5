...
//     <TouchableOpacity
//       style={styles.checklistItem}
//       onPress={() => navigation.navigate(ROUTES.host_edit_checklist, {
...
//       </View>
//       <View style={styles.checklistMeta}>
//         <Text style={styles.checklistMetaText}>⏱️ {minutesToDuration(item.totalTime)}</Text>
//         <Text style={styles.checklistMetaText}>💰 ${item.totalFee?.toFixed(2)}</Text>
//       </View>
//       <MaterialIcons name="chevron-right" size={20} color={COLORS.gray} style={styles.checklistArrow} />
//     </TouchableOpacity>
...
export default function PropertyDashboard({ route }) {
  ...
  // ─── Handle route params on focus ──────────────────────────────
  useFocusEffect(
    useCallback(() => {
      const { refresh, message, status } = route.params || {};

      // If refresh flag is present, reload data and optionally show an alert
      if (refresh) {
        fetchAll();
        // Clear the refresh param to avoid repeated reloads
        navigation.setParams({ refresh: undefined });

        // If a success message is provided, show it once
            ...
            [
              {
                text: tSafe('ok', 'OK'),
                onPress: () => {
                  // Clear the message params so it doesn't show again
                  navigation.setParams({ message: undefined, status: undefined });
      ...
      // If there are stale params, clean them up
      if (route.params?.propertyId !== undefined) {
        navigation.setParams({ propertyId: undefined });
      }
      if (route.params?.timestamp !== undefined) {
  ...
  const handleAddChecklist = () => {
    navigation.navigate(ROUTES.host_create_checklist, {
      ...
      propertyId: property._id,
      apartmentName: property.apt_name,
    });
  };
