...
//   const cleanerFee = useMemo(() => {
//     if (!assignedTo || !Array.isArray(assignedTo) || !cleaner?._id) return 0;
  
//     const cleanerIdStr = String(cleaner._id);
  
//     const assignedEntry = assignedTo.find(item =>
//       String(item.cleanerId) === cleanerIdStr ||
//       (Array.isArray(item.acceptedCleaners) && item.acceptedCleaners.some(id => String(id) === cleanerIdStr))
//     );
...
//                   starStyle={{ marginHorizontal: 0 }}
//                 />
//                 <Text style={{marginLeft:8}}>{cleaner.averageRating}</Text>
//               </View>
//           </View>
//           <View>
...
//                 {tSafe('see_all_reviews', 'See all {count} reviews', { count: cleaner.totalRatings })}
//               </Text>
//             }
//           </View>
//         </View>
...
//     {[
//       { key: 1, label: tSafe('tab_about', 'About') },
//       { key: 2, label: tSafe('tab_availability', 'Availability') },
//       { key: 3, label: tSafe('tab_license', 'License') },
//       { key: 4, label: tSafe('tab_portfolio', 'Reviews') },
//     ].map((tab) => (
//       <TouchableOpacity
//         key={tab.key}
//         style={[
//           styles.chip,
//           currentStep === tab.key && styles.chipActive,
//         ]}
//         onPress={() => setCurrentStep(tab.key)}
//         activeOpacity={0.7}
//       >
//         <Text
//           style={[
//             styles.chipText,
//             currentStep === tab.key && styles.chipTextActive,
//           ]}
//         >
//           {tab.label}
//         </Text>
...
//     flexDirection: 'row',
//     alignItems: 'center',
//     gap: 10, // works on iOS, for Android use marginRight on each chip
//   },
//   chip: {
...
const CleanerProfilePay = ({ navigation }) => {
  ...
  const fetchAvailability = async () => {
    try {
      const response = await userService.getCleanerAvailability(cleaner._id);
      const data = response.data.data;
      setAvailabilityData(data);
    } catch (err) {
      console.error('Error fetching availability:', err);
      // Silently fail – we'll just not show availability
    }
  };
    ...
    <SafeAreaView style={styles.safeContainer}>
      ...
      <View style={styles.avatar_background}>
        ...
        <Text style={styles.location}>
          {cleaner.location?.city || tSafe('unknown', 'Unknown')}, {cleaner.location?.region_code || tSafe('unknown', 'Unknown')}
        </Text>
          ...
          <View style={styles.ratingRow}>
            <View style={styles.ratingLeft}>
              ...
              <Text style={styles.ratingNumber}>
                {cleaner.averageRating ? cleaner.averageRating.toFixed(1) : '0.0'}
              </Text>
              <Text style={styles.reviewCount}>
                ({cleaner.totalRatings || 0} {tSafe('reviews', 'reviews')})
              </Text>
            </View>
            {cleaner.totalRatings > 0 && (
              <TouchableOpacity onPress={openReviews}>
                <Text style={styles.seeAllReviews}>{tSafe('see_all_reviews', 'See all')}</Text>
              </TouchableOpacity>
            )}
          </View>

          {/* Chips Tabs - now with Reviews replacing Portfolio */}
          <View style={styles.chipsContainer}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsScroll}>
              {[
                { key: 1, label: tSafe('tab_about', 'About') },
                { key: 2, label: tSafe('tab_availability', 'Availability') },
                // { key: 3, label: tSafe('tab_license', 'License') },
                { key: 4, label: tSafe('tab_reviews', 'Reviews') }, // 🟢 Changed from "Portfolio" to "Reviews"
              ].map((tab) => (
                <TouchableOpacity
                  key={tab.key}
                  style={[
        ...
        <View style={styles.contentPadding}>
          {currentStep === 1 && (
            <Animatable.View animation="fadeIn" duration={600}>
              <AboutMeDisplay mode="display" aboutme={item.aboutme} />
              {item.certification && Object.keys(item.certification).length > 0 && (
                <View style={styles.certificationSection}>
                  {/* <Text style={styles.certificationTitle}>
                    {tSafe('certifications_license', 'Certifications & License')}
                  </Text> */}
               
                  {/* Certifications section */}
                  {item.certification && Object.keys(item.certification).length > 0 && (
                    <HostCertificationDisplay certification={item.certification} />
                  )}
                </View>
              )}
            </Animatable.View>
          ...
          {currentStep === 2 && (
            <Animatable.View animation="fadeIn" duration={600}>
              {/* {item.availability && Object.keys(item.availability).length > 0 && (
                <HostAvailabilityDisplay availability={item.availability} />
              )} */}
              {availabilityData && (
                <HostAvailabilityDisplay
                  availability={availabilityData.availability || []}
                  bookedSchedules={availabilityData.booked_schedules || []}
                />
              )}
              
              {/* <AvailabilityDisplay mode="display" availability={item.availability} /> */}
            </Animatable.View>
          )}
          {currentStep === 4 && (
            <Animatable.View animation="fadeIn" duration={600} style={styles.reviewsTabContent}>
              {reviews.length > 0 ? (
                <Reviews ratings={reviews} cleanerId={item._id} />
              ) : (
                <View style={styles.noReviewsContainer}>
                  ...
                  <Text style={styles.noReviewsText}>
                    {tSafe('no_reviews_yet', 'No reviews yet for this cleaner.')}
                  </Text>
                </View>
              )}
            </Animatable.View>
      ...
      {/* Reviews Modal – still accessible via "See all" link */}
      <Modal 
        isVisible={visible} 
        onSwipeComplete={() => setVisible(false)} 
        swipeDirection="down"
...
const styles = StyleSheet.create({
  ...
  avatarWrapper: {
    ...
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: COLORS.primary, // match background to avoid gaps
    shadowColor: '#000',
