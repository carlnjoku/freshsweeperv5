  ...
  const handleCreateChecklist = useCallback(() => {
    if (!formData.aptId) {
      Alert.alert(
        ...
        tSafe('select_apartment_first', 'Please select an apartment first')
      );
      return;
    }
  ...
  return (
    <View>
      <Text style={{ fontSize: 24, fontWeight: 'bold' }}>
        {tSafe('choose_property', 'Choose Your Property')}
      </Text>
      <Text style={{ fontSize: 14, marginBottom: 20, color: COLORS.gray }}>
        {tSafe('select_apartment_desc', 'Select the Apartment You Want to Schedule for Cleaning')}
      </Text>

      <FloatingLabelPickerSelect
        label={tSafe('select_apartment', 'Select Apartment')}
        value={formData?.aptId}
        onValueChange={(value) => {
          console.log("[PropertyDetails] Apartment selected:", value);
          const selected = ownerApartments.find((apt) => apt._id === value);
      ...
      {checkingChecklist && (
        <View style={styles.checklistChecking}>
          ...
          <Text style={styles.checklistText}>
            {tSafe('checking_checklist', 'Checking for checklist...')}
          </Text>
        </View>
      )}
        ...
        <View style={styles.checklistStatus}>
          {checklistStatus.hasChecklist ? (
            <View style={styles.checklistAvailable}>
              ...
              <View style={styles.checklistInfo}>
                <Text style={styles.checklistName}>
                  {tSafe('checklist_prefix', 'Checklist:')} {checklistStatus.checklistName}
                </Text>
                <Text style={styles.checklistTasks}>
                  {checklistStatus.checklistTasks?.length || 0}{' '}
                  {tSafe('tasks_configured', 'tasks configured')}
                </Text>
              </View>
            </View>
          ) : (
            <>
              <View style={styles.checklistMissing}>
                ...
                <View style={styles.checklistInfo}>
                  <Text style={styles.checklistWarning}>
                    {tSafe('no_checklist_found', 'No checklist found for this property')}
                  </Text>
                  <Text style={styles.checklistInstruction}>
                    {tSafe('create_checklist_instruction', 'You need to create a checklist before scheduling cleaning')}
                  </Text>
                </View>
              </View>
              <TouchableOpacity
      ...
      <View style={styles.note}>
        <Text style={styles.noteText}>
          {tSafe('property_note', 'Note: Each property must have a cleaning checklist before scheduling.')}
        </Text>
      </View>
    </View>
  );
