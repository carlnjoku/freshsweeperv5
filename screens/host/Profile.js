...
const Profile = () => {
  ...
  const confirmDeleteAccount = async () => {
    ...
    try {
      ...
      // 2. Optionally, remove Firebase data (if you store user data there)
      // const userRef = ref(db, `users/${currentUserId}`);
      // await remove(userRef);

      // 3. Logout – clears token, storage, and navigates to Public
      await logout();

      // 4. Show success (may not be visible because of logout)
      Alert.alert(
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {editMode ? (
          ...
          <>
            ...
            {/* Delete Account Button – always shown when not editing */}
            <Button
              mode="contained"
              buttonColor="#dc3545"
