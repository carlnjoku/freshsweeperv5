...
const Messages = ({navigation}) => {
  ...
  const renderHeader = () => (
    <View style={styles.header}>
        ...
        <View>
          ...
          <Text style={styles.headerSubtitle}>
            {totalUnreadCount > 0 
              ...
              : tSafe('all_caught_up', 'All caught up!')
            }
          </Text>
        </View>
        {/* <TouchableOpacity style={styles.newChatButton}>
      ...
      >
        ...
        <TextInput
          style={styles.searchInput}
          placeholder={tSafe('search_conversations_placeholder', 'Search conversations...')}
          placeholderTextColor="#9CA3AF"
          value={searchQuery}
          onChangeText={setSearchQuery}
          onFocus={() => setIsSearchFocused(true)}
  ...
  const emptyListing = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyTitle}>
        {searchQuery ? tSafe('no_conversations_found', 'No conversations found') : tSafe('no_messages_yet', 'No messages yet')}
      </Text>
      <Text style={styles.emptyMessage}>
        {searchQuery 
          ? tSafe('adjust_search_message', 'Try adjusting your search or filter to find what you\'re looking for.')
          : tSafe('no_messages_description', 'When you start chatting with cleaners or hosts, your conversations will appear here.')
        }
      </Text>
      {!searchQuery && (
        <TouchableOpacity 
          ...
          onPress={() => navigation.navigate(ROUTES.cleaner_dashboard)}
        >
          <Text style={styles.emptyButtonText}>{tSafe('find_cleaners', 'Find Cleaners')}</Text>
        </TouchableOpacity>
      )}
    </View>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        <View style={styles.loaderContainer}>
          <ActivityIndicator size="large" color={COLORS.primary} />
          <Text style={styles.loadingText}>{tSafe('loading_conversations', 'Loading your conversations...')}</Text>
        </View>
      ) : (
        <FlatList
          data={filteredConversations}
